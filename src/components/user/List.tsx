		...
		<div className="w-full px-[16px] pt-[24px]">
			<div className="h-[54px] w-full flex items-center justify-between">
				<div className="flex gap-[16px] text-[16px] text-[#67646B]">
					<div className={active === 0 ? "text-[#FFF] f600 cursor-pointer" : "cursor-pointer"} onClick={() => handleClick(0)}>我的持仓</div>
					<div className={active === 1 ? "text-[#FFF] f600 cursor-pointer" : "cursor-pointer"} onClick={() => handleClick(1)}>我创建的</div>
					<div className={active === 2 ? "text-[#FFF] f600 cursor-pointer" : "cursor-pointer"} onClick={() => handleClick(2)}>收益代币</div>
				</div>
			</div>
			{
				!loading && Array(5).fill(0).map((item, index) => (
